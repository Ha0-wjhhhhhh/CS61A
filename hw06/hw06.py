...
def store_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert n > 0, "only positive digits should be passed"
    if n < 10:
        return Link(n)
    else:
        #new_rest = Link(n%10)
        #new_first = store_digits(n // 10)
        #digits = new_first
        #while new_first.rest is not Link.empty:
            #new_first = new_first.rest
        #new_first.rest = new_rest
        #return digits
        list_digits = []
        while n > 0:
            list_digits.append(n % 10)
            n //= 10
        link_digits = Link(list_digits[0])
...
def two_list(vals, amounts):
    ...
    "*** YOUR CODE HERE ***"
    #link = Link.empty
    #for val, amount in zip(vals[::-1], amounts[::-1]):
    ...
    if len(vals) == 0:
        ...
        link = two_list(vals[1:], amounts[1:])
        for _ in range(amounts[0]):
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            next_fib = VirFib(self.value + self.previous.value)
            next_fib.previous = self
            return next_fib
...
def is_bst(t):
    ...
    if len(t.branches) == 0:
        ...
        return is_bst(t.branches[0]) and is_bst(t.branches[1]) and t.branches[0].label <= t.branches[1].label and bst_max(t.branches[0]) <= t.label <= bst_min(t.branches[1])
    else:
        return False

def bst_max(t):
    assert is_bst(t) is True, "only a bst can be passed to the function"
    bMax = t.label
    for branch in t.branches:
        if bst_max(branch) > bMax:
            bMax = bst_max(branch)
    return bMax
