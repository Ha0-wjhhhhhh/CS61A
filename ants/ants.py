...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
            # BEGIN Problem 8
        elif place.ant.is_container or self.is_container:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            for i,bee in enumerate(self.place.bees[:]):
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        if self.is_container and self.ant_contained is Nont and other.is_container is not True:
            return True
        else:
            return False
        # END Problem 8
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while place is not None and place.is_hive is not True:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared_length > 0:
            if (self.place.is_hive is True) or (self.slowed_length > 0 and gamestate.time % 2 == 1):
                destination = self.place
            else:
                destination = self.place.entrance
                self.scared_length -= 1
        elif self.slowed_length > 0:
            if (self.place.is_hive is True) or (gamestate.time % 2 == 1):
                destination = self.place
            else:
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 1
        "*** YOUR CODE HERE ***"
        for key, bee in enumerate(self.place.bees[:]):
            bee.reduce_health(self.damage)
        # END Problem Optional 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while (place.entrance.is_hive is not True) and (place.entrance is not None) and (place is not None):
            place = place.entrance
            distance += 1
            if place.ant is not None:
                insects[place.ant] = distance 
