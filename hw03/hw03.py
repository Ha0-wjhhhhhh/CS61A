...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count(change, largest):
        if change < 0:
            return 0
        else:
            if not descending_coin(largest):
                return 1
            else:
                return count(change, descending_coin(largest)) + count(change - largest, largest)
...
from operator import eq, sub, mul
