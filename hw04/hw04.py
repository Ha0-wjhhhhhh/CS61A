...
def balanced(m):
    ...
    if is_mobile(m):
        ...
        assert is_planet(m),"balanced(m) must get a planet or mobile"
        return  True 
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m), "totals_tree must get a mobile or planet"
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_tree(t), "replace_loki_at_leaf must get a tree and a lokis_replacement"
    assert type(lokis_replacement) == str, "replace_loki_at_leaf must get a tree and a lokis_replacement"
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0, "interva' not span zero"
