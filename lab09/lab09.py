...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        ret = subseqs(s[1:])
        return insert_into_all(s[0], ret) + ret
...
def trade(first, second):
    ...
    while equal_prefix() is not True and m <= len(first) and n <= len(second):
...
def insert(link, value, index):
    ...
    if index == 0:
        if link.rest is not Link.empty:
            insert(link.rest, link.first, index)
        else:
...
def deep_len(lnk):
    ...
    if lnk is Link.empty:
        return 0
    elif  not isinstance(lnk, Link) or (lnk.rest is Link.empty and not isinstance(lnk.first, Link)):
        return 1
    else:
...
def long_paths(t, n):
    ...
    if t.is_leaf():
        ...
        for b in t.branches:
            if long_paths(b, n-1):
                pathes.extend([[t.label] + path for path in long_paths(b, n-1)])
        return pathes
