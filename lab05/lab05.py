def factors_list(n):
    ...
    [1, 2, 4, 7, 14]
    """
    all_factors = [i for i in range(1,n) if not n%i]
    "*** YOUR CODE HERE ***"
    return all_factors
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_a = distance(city_a,dst)
    distance_b = distance(city_b,dst)
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        leaves2branches = [tree(leaf) for leaf in leaves]
        return tree(label(t), leaves2branches)
    else:
        new_branches = []
        for branch in branches(t):
