...
def accuracy(typed, reference):
    ...
    for i in range(min(len(typed),len(reference))):
        if typed[i] == reference[i]:
             correct_times += 1
    return correct_times/len(typed)*100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    if (typed_word in valid_words) and typed_word:
        return typed_word
    most_similar, least_difference = valid_words[0], diff_function(typed_word,valid_words[0],limit)
    for word in valid_words:
        if diff_function(typed_word,word,limit) < least_difference:
            most_similar, least_difference = word, diff_function(typed_word,word,limit)
    if least_difference > limit:
        return typed_word
    else:
        return most_similar
...
def feline_flips(start, goal, limit):
    ...
    5
    >>> feline_flips("jose", "hello", big_limit)   # Substitute: r->h, o->e, s->l, e->l, length difference of 1.
    5
    """
    # BEGIN PROBLEM 6
    if limit < 0:
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit < 0:  # Fill in the condition
    ...
    elif start == goal or (not start) or (not goal):  # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        return abs(len(start) - len(goal))
        # END
        ...
        if goal[0] == start[0]:
            ...
            add = minimum_mewtations((goal[0] + start)[1:],goal[1:],limit-1)
            remove = minimum_mewtations(start[1:],goal,limit-1)
            substitude = minimum_mewtations((goal[0] + start[1:])[1:],goal[1:],limit-1)
            return min(add,remove,substitude)+1        
        # END
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        ...
        acr += 1
    acr /= len(prompt)            
    upload({'id':user_id, 'progress':acr})
    print(acr)
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [ [timestamp_list[i+1] - timestamp_list[i] for i in range(len(timestamp_list)-1)]\
                for timestamp_list in times_per_player] 
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            ...
            for k in player_indices:
                if get_times(match)[i][j] > get_times(match)[k][j] or (get_times(match)[i][j] == get_times(match)[k][j] and i > k):
                    fastest = False
            if fastest:
                fastest_list.append(get_words(match)[j])
        ret.append(fastest_list)
    return ret
    # END PROBLEM 10
